...
class nn_convolutional_layer:
    ...
    #######
    def forward(self, x):
        ft, d, f_h, f_w = self.W.shape
        arr = []
        brr = []
        for tensor3 in x:
            for k in range(ft):
                window = view_as_windows(tensor3, (d, f_h, f_w))
                window = window.squeeze(0)  # batch 차원 1로 고정되니까 제거
                a1, a2, a3, a4, a5 = window.shape
                window = window.reshape(a1, a2, -1)
                filt = self.W[k]
                filt = filt.reshape(d * f_h * f_w, -1)
                res = torch.matmul(window, filt)
                res = res.squeeze(-1)
                arr.append(res)
            result = torch.stack(arr)
            arr = []
            brr.append(result)
        res = torch.stack(brr)
        out = res + self.b  # broadcasting
...
class nn_max_pooling_layer:
    ...
    #######
    def forward(self, x):
        bt, d, in_w, in_h = x.shape
        # print(x.shape)
        arr = []
        for tensor3 in x:
            window = view_as_windows(tensor3, (d, self.pool_size, self.pool_size), step=self.stride)
            window = window.squeeze(0)
            a1, a2, a3, a4, a5 = window.shape
            window = window.reshape(a1, a2, a3, -1)
            window, _ = window.max(dim=-1, keepdim=True)
...
if __name__ == "__main__":
    ...
    for i in range(num_test):
        ...
        with torch.no_grad():
            ...
            out = cnv.forward(x)
            W,b = cnv.get_weights()
            test_conv_layer.weight = torch.nn.Parameter(W)
            test_conv_layer.bias = torch.nn.Parameter(torch.squeeze(b))
    ...
    for i in range(num_test):
        ...
        test_pooling_layer = torch.nn.MaxPool2d(kernel_size=(pool_size,pool_size), stride=stride)
