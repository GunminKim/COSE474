...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        ft, a2, a3, a4 = self.W.shape
        arr = []
        brr = []
        for tensor3 in x:
            for k in range(ft):
                # print(tensor3.shape)
                temp = tensor3.squeeze(0)
                # print(temp.shape)
                window = view_as_windows(temp, (a3, a4))
                # print(window.shape)
                b1, b2, b3, b4 = window.shape
                window = window.reshape(b1, b2, -1)
                # print(window.shape)
                filt = self.W[k].squeeze(0)
                # print(filt.shape)
                filt = filt.reshape(a3 * a4, -1)
                # print(filt.shape)
